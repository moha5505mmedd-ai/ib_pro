...
def create_vector_db_with_docling(pdf_path: str, db_directory: str = None) -> bool:
    ...
    try:
        ...
        pipeline_options.do_ocr = True # تفعيل OCR
        pipeline_options.ocr_options = RapidOcrOptions(lang=["ar", "en"])
...
async def retrieve_node(state: AgentState) -> dict:
    ...
    async def search_videos_task():
        ...
        if search_mode in ["video", "both"] and TL_API_KEY and TL_INDEX_ID and tl_client:
            try:
                ...
                # التحديث الجذري لدعم Pegasus 1.5
                    analysis = await asyncio.to_thread(
                        tl_client.analyze,
                        model_name="pegasus1.5",
                        video=VideoContext_AssetId(asset_id=clip.video_id),
                        prompt_v_2=AnalyzePromptV2(
                            input_text=f"هل يحتوي هذا المقطع على إجابة السؤال: {question}؟ أجب بنعم أو لا ثم قدم الشرح."
                            #input_text=f"هل يحتوي هذا المقطع على إجابة السؤال: '{question}'؟ أجب بنعم أو لا، ثم قدم الشرح. أمر إجباري: يجب أن تكتب إجابتك وشرحك باللغة العربية حصراً."
                        )
                    )
    ...
    # 1. تشغيل مساري البحث في نفس الوقت
    docs_results, videos_results = await asyncio.gather(
        search_docs_task(),
        search_videos_task()
    )
    ...
    # 2. بناء قسم المستندات *فقط* إذا كان نمط البحث يطلبه
    if search_mode in ["docs", "both"]:
        docs_context = "\n---\n".join(docs_results) if docs_results else "لم يتم العثور على إجابة في المستندات."
        final_context_parts.append(f"<DOCUMENTS_SECTION>\n{docs_context}\n</DOCUMENTS_SECTION>")

    # 3. بناء قسم الفيديو *فقط* إذا كان نمط البحث يطلبه
    if search_mode in ["video", "both"]:
        videos_context = "\n---\n".join(videos_results) if videos_results else "لم يتم العثور على شروحات مرئية."
        final_context_parts.append(f"<VIDEO_SECTION>\n{videos_context}\n</VIDEO_SECTION>")

    # 4. دمج الأقسام المطلوبة فقط
    final_context = "\n\n".join(final_context_parts)
    
    return {"context": final_context, "question": question}
    # 1. تشغيل مساري البحث في نفس الوقت (لا تحذف هذا السطر)
    #docs_res, videos_res = await asyncio.gather(search_docs_task(), search_videos_task())

    # 2. دمج المستندات والفيديوهات كل على حدة
    #docs_context = "\n---\n".join(docs_res) if docs_res else "لم يتم العثور على أي معلومات بخصوص هذا السؤال في المناهج والمستندات."
    #videos_context = "\n---\n".join(videos_res) if videos_res else "لم يتم العثور على أي شروحات مرئية لهذا السؤال في الفيديوهات."

    # 3. التغليف الصارم للسياق (Strict Context Delimiters)
    #final_context = f"""
    #<DOCUMENTS_SECTION>
    #{docs_context}
    #</DOCUMENTS_SECTION>
...
async def generate_node(state: AgentState, config: RunnableConfig): 
    ...
    # ==========================================
    # 3. تجهيز القوالب الهيكلية (Templates)
    # ==========================================
    math_template = """
📌 السؤال
═════════════════════
...
════════════════════════
① [اسم الخطوة]
[القانون الرياضي والتعويض بالأرقام الجديدة كما هي في السياق]
──────────────────────────────────────
══════════════════════════════
📚 المراجع
═══════════════════════════════════
...
"""
    ...
    # ==========================================
    # 4. تعليمات التنسيق بناءً على المسار
    # ==========================================
    if search_mode == "both":
        format_instructions = f"""
يجب عليك تحليل السياق المسترجع، وتقسيم إجابتك بشكل إجباري إلى قسمين واضحين:

**القسم الأول: الإجابة من المستندات**
استخرج إجابة المستندات فقط من الجزء المحصور بين <DOCUMENTS_SECTION> و </DOCUMENTS_SECTION>. إذا كان القسم يشير إلى عدم وجود معلومات، اكتب بوضوح: 'لا توجد معلومات في المناهج'.

(اكتب هنا الشرح المستخرج من النصوص التي تبدأ بكلمة [المستند: ...])
- استخدم القوالب التالية بناءً على نوع السؤال (نظري أم رياضي):
قالب المسائل الرياضية:
{math_template}
قالب الأسئلة النظرية:
{theory_template}

**القسم الثاني: الإجابة من الفيديو**
...
- 🎥 التعامل مع الفيديو: يجب عليك إدراج جميع أزرار <a class="mux-jump-btn" ...> الموجودة في السياق كما هي كنص عادي دون أي تعديل.
"""
    else: # مسار docs
        format_instructions = f"""
قم بصياغة الإجابة بناءً على المستندات النصية المسترجعة في السياق فقط.
قواعد التنسيق للمستندات:
١. التجميع الشامل (مهم جداً): ابحث في كل النصوص المسترجعة واجمع كل الجوانب المتعلقة بالسؤال وضعها في نقاط متعددة ومستقلة.
٢. تصنيف السؤال: إذا كان يحتوي على أرقام ويطلب حسابات، استخدم "قالب المسائل الرياضية". إذا كان يسأل عن تعريف أو شرح، استخدم "قالب الأسئلة النظرية".

قالب المسائل الرياضية (التزم بالهيكل والرموز):
{math_template}

قالب الأسئلة النظرية (التزم بالهيكل والرموز):
{theory_template}
"""

    # ==========================================
    # 5. التوجيهات الحديدية الأساسية (Core Prompt)
    # ==========================================
    system_prompt_text = f"""أنت مساعد ذكي مخصص لاستخراج المعلومات من المستندات والفيديوهات المرفقة فقط لمنصة DKM.

🔥 قواعد حديدية لا تقبل التجاوز أبداً (Zero-Tolerance for Hallucination):

1. **الالتزام الحرفي بالسياق:** استخرج الإجابة من "السياق المسترجع" فقط. يُمنع منعاً باتاً إضافة أي معلومات، استنتاجات، قوانين رياضية، أمثلة، أو شروحات من ذاكرتك العامة مهما كانت صحيحة.
2. **النسخ الأعمى للنصوص المبتورة:** إذا كان المقطع المسترجع ينتهي فجأة قبل ذكر القانون الرياضي أو المعلومة، يُمنع منعاً باتاً إكمال الناقص من ذاكرتك. قم بعرض النص المبتور كما هو بالضبط وتوقف.
3. **الرموز والرياضيات:** استخدم نفس الرموز الرياضية والصيغ المذكورة في النص حرفياً. في المسائل الجديدة، استخرج القانون من النص وقم بالتعويض بالأرقام الجديدة خطوة بخطوة. يمنع حل مسألة لا يوجد قانونها أو مثال مشابه لها في السياق.
4. **الدقة في التوثيق:** عندما تذكر معلومة، يجب أن ترفق معها اسم الملف ورقم الصفحة الصحيح [صفحة: X] كما هو وارد في السياق. إياك أن تخمن المراجع.
5. **آلية الرفض:** إذا سألك الطالب عن شيء والمقطع المسترجع لا يحتوي على إجابة، يجب عليك الرد حرفياً: "عذراً، لم أتمكن من العثور على معلومات متطابقة أو كاملة في السياق المسترجع."
6. **حظر التلخيص المُخل (No Over-Summarization):** عندما تستخرج تعريفاً أو شرحاً (مثل تعريف معامل الارتباط ومقاييسه)، يُمنع منعاً باتاً اختصاره أو تلخيصه. يجب عليك نقل الفقرة كاملة بجميع تفاصيلها وأرقامها (مثل +1، -1، 0.90) كما وردت في السياق المسترجع لتكون الإجابة الأكاديمية وافية.
7. **التفرقة بين (مجرد الذكر) و (التعريف) - [درع الهلوسة]:** إذا سألك الطالب عن مصطلح (مثل: الانحدار الخطي البسيط)، ووجدت هذا المصطلح في السياق مجرد "عنوان" أو "ذكر عابر" بدون شرح تفصيلي أو قوانين، **يُمنع منعاً باتاً** أن تقوم بتأليف الشرح والقوانين من ذاكرتك. يجب عليك التوقف فوراً والقول: "عذراً، المصطلح مذكور في السياق كعنوان أو ذكر عابر، ولكن الشرح التفصيلي غير متوفر في المستندات المسترجعة."

تعليمات حديدية:عندما يطلب منك المستخدم إجابة بناءً على المستندات والفيديو (وضع both)، يجب عليك الالتزام الصارم بالتالي:


استخرج إجابة الفيديو فقط من الجزء المحصور بين <VIDEO_SECTION> و </VIDEO_SECTION>.

ممنوع منعاً باتاً دمج معلومات الفيديو في قسم المستندات، أو العكس

🖼️🎥 قواعد التعامل مع الوسائط (الصور والفيديوهات):
- إياك أن تخترع أو تؤلف رابط صورة من عندك.
- اطبع كود HTML للصورة (<img src="...">) أو زر الفيديو (<a class="mux-jump-btn" ...>) فقط وحصرياً إذا كان مكتوباً حرفياً أمامك داخل السياق المسترجع ليتمكن الطالب من مشاهدته.
- ⚠️ تحذير: يُمنع استخدام تنسيق الأكواد الماركداون (```html) عند طباعة الصور أو أزرار الفيديو. ضعها مباشرة في الرد العادي لكي يتعرف عليها المتصفح ويعرضها كعنصر بصري وليس كنص برمجي.

📋 تعليمات التنسيق والهيكلة (بناءً على مسار البحث الحالي):
{format_instructions}
